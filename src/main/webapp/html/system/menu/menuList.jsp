	  ...
	  <tbody>
	  	<c:forEach items="${menuList}" var="menu">
	  		...
	  		>
		  		...
		  		<td>${menu.menuUrl}</td>
		  		<td>
		  			<a href="javascript:;" class="btn btn-sm blue" onclick="update('/menu/menu?id=${menu.id}')">
		  				<i class="fa fa-edit"></i>编辑
		  			</a>
						<a href="javascript:;" class="btn btn-sm red" onclick="del('/menu/delete?id=${menu.id}')">
					...
					success: function(data){
						...
						for(var i = 0; i < data.length; i++){
							...
							rows += '<tr id="'+data[i].id+'" pId="'+data[i].parentId+'" '+hasChild+'>\n'
								...
								+	'<td>\n'
								+	'<a href="javascript:;" class="btn btn-sm blue" onclick="update(\'/menu/menu?id='+data[i].id+'\')">'
								+	'<i class="fa fa-edit"></i>编辑</a>\n'
								+	'<a href="javascript:;" class="btn btn-sm red" onclick="del(\'/menu/delete?id='+data[i].id+'\')">'
